...
public class MyResource {
	...
	@GET
	@Path("/contents/topic/{topic}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getContentByTopic(@PathParam("topic") String topic){
			readFile();
			list.clear();
			for(Content c : contents.values()) {
				if(c.getTopic().contains(topic)) {
					list.add(c);
				}	
			}
	...
	@Path("/contents/desc/{desc}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getContentById(@PathParam("desc") String description){
			readFile();
			list.clear();
			description.replaceAll("%20", " ");
			for(Content c : contents.values()) {
	...
	@Path("/server/{server}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response recoverServer(@PathParam("server") String server){	 
		readFile();
		list.clear();
		for(Content c : contents.values()) {
			if(c.getServer().contains(server)){
				list.add(c);
			}	
		}
		return Response.status(201).entity(list).build();
	...
	@PUT
	@Path("/content/{client}/{id}/edit")
	@Produces(MediaType.APPLICATION_JSON)
	public Response updateDescription(@PathParam("client") String client, @PathParam("id") String id, String description){
		try {
			readFile();
	...
	public Response deleteContent(@PathParam("id") String id){
		try {
			readFile();
			contents.remove(Integer.parseInt(id));
			writeFile();
